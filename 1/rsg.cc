...
/* Recursively generates texts. Loops through the current production,
   if the string in the production is not present in the "grammar" map,
   this means that the string is a terminal and calls a recursive function
   for the strings respective production.
   If the string is present in the "grammar" map, prints the said string. */
void printTxt( Production prod, map< string, Definition >& grammar ){
  for( Production::iterator it = prod.begin(); it != prod.end(); it++){

    if( grammar.find(*it) != grammar.end() ){
      Production newProd = grammar[*it].getRandomProduction();
      printTxt( newProd, grammar );
    } 
    else{
      cout << " " << *it;
    }   
...
int main(int argc, char *argv[])
{
  ...
  // Gets the text template and generates 3 random texts
  Production start = grammar["<start>"].getRandomProduction();
  for(int i = 1; i < 4; i++){
    cout << "Version #" << i << "------------------\n";
    printTxt( start, grammar );
    cout << endl << endl;
  }
