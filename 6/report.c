...
{
  // locks the report to so that only one worker can report at a time
...
Report_DoReport(Bank *bank, int workerNum)
{
  ...
  bank->reportsLeftToDo--;
  // if the worker does not report last, makes them wait until the last one makes a report.
  // prevents workers to start the next day without everyone's report
  if( bank->reportsLeftToDo != 0 ){

    pthread_mutex_unlock( &bank->reportLock );
    // decreases semaphore numWorkers-1 times
    sem_wait( &bank->nextDayLock );
  ...
  if (rpt->numReports >= MAX_NUM_REPORTS) {
      ...
      bank->reportsLeftToDo = bank->numWorkers;
      // used by last worker that reports, gives others permission to start the next day
      for(int i = 0; i < bank->numWorkers-1; i++){
        sem_post( &bank->nextDayLock );
      }
      pthread_mutex_unlock( &bank->reportLock );
